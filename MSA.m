clc
clear
%S1=fastaread('\strong_promoter.txt');
%S2=fastaread('\weak_promoter.txt');
%S3=fastaread('\non_promoter.txt');
%S=[S1;S2;S3];
    ...
    for g = 1:G
        for s = 1:15
            for t = 1:15
                ...
                C1=value_matrix(:,s);
                C2=value_matrix(:,t);       
                t1=sum((Csg-mean(C1)).*(Ctg1-mean(C2)));
                l1=(length(Seq)-1-g);
