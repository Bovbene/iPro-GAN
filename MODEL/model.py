...
Param: 	self.trainable = True, self.batch_size = 50, self.lr = 0.0002, self.mm = 0.5 ,
        ...
        self.Threshold = 0.00               ----(float) The lower-bound acc to save model
        self.x_data = feature               ----(numpy array) The inpuuted data
        self.y_data = label                 ----(numpy array) The outputed data
        self.train_rate = 0.7               ----(float) Proportion of training set.
...
class Model():
    ...
    def build_model(self):
        ...
        self.t_x = tf.placeholder(tf.float32,shape = [1,self.feature_dim*1],name = 'Test_D_Input')
        ...
        if self.model == 'DCGAN':
            ...
            return
        test_logits, _ = self.discriminator('dis', self.t_x, reuse=True)
        ...
        for epoch in range(self.EPOCH):
            ...
            for idx,(batchx,batchl) in enumerate(zip(xtrain_li,ytrain_li)):
                ...
                if ((idx+1) % 100) == 0:  # flush plot picture per 1000 iters
                    plot.flush()
                plot.tick()
            Acc,Se,Sp,Mcc,train_label,train_Score = self.test(np.vstack(xtest_li),np.vstack(ytest_li),mark = "Feature n' Label")
            ...
            if Acc > self.Threshold:
                AucPlt1('Auc Plot',train_label,train_Score)
                ...
                self.Threshold = Acc
                self.Threshold_Se = Se
                self.Threshold_Sp = Sp
                self.Threshold_Mcc = Mcc
        
        fn_li = glob('./RESULT/*.npz')
        for fn in fn_li:
            file = os.path.split(fn)[-1]
            thershold_old = float(file.split('_')[-1].replace('.npz','')) if self.remark in file else float('Inf')
            if self.Threshold>thershold_old:
                os.remove(fn)
                np.savez('./RESULT/'+self.remark+'_'+str(self.Threshold)+'.npz',train_label = train_label,train_Score = train_Score)
        if thershold_old == float('Inf'):
            np.savez('./RESULT/'+self.remark+'_'+str(self.Threshold)+'.npz',train_label = train_label,train_Score = train_Score)
            #with np.load(fn) as train_data
        train_data = {}
        fn_li = glob('./RESULT/*.npz')
        for i,fn in enumerate(fn_li):
            with np.load(fn) as data:
                train_data['train_label{}'.format(i)] = data['train_label']
                train_data['train_Score{}'.format(i)] = data['train_Score']
        try:
            AucPlt2('ROC',train_data['train_label0'],train_data['train_Score0'],train_data['train_label1'],train_data['train_Score1'])
        except KeyError:
            pass
    ...
    def bn(self, name, input):
        return tf.contrib.layers.batch_norm(input, decay=0.9,
    ...
    def test(self,xtest_li,ytest_li,mark = ''):
        ...
        for i,(testx, textl) in enumerate(zip(xtest_li,ytest_li)):
            test_logits,pre_logits = self.sess.run([self.test_logits,self.pre_logits], feed_dict={self.t_x:testx.reshape((1,-1))})
            test_logits = test_logits[:,0:2]
            test_label = np.argmax(textl)
    ...
    def TestNewSet(self):
        ...
        Acc,Se,Sp,Mcc,train_label,train_Score = self.test(np.vstack(xtrain_li),np.vstack(ytrain_li),mark = "Feature n' Label")
